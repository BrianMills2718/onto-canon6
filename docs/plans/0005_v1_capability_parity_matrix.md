...
| Canonical concept/belief graph with system beliefs | `concept_storage.py`, `beliefs`/`concepts` schema, `sys:*` beliefs | Not yet recovered; current durable subject is still the reviewed candidate assertion | Deferred | Phase 11 |
| Stable identity and external references, including Q-code-like cross-investigation identity | Wikidata/Q-code flow in v1 | Not yet recovered as a successor-local identity subsystem | Deferred | Phase 12 |
